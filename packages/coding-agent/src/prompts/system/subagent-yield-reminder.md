...
1. **Assignment complete — yield now** — if the assignment you were given is finished, call terminal `yield` immediately: omit `type` for the single final structured result in `result.data`, or use `type: string` to finalize from your last assistant turn. This reminder NEVER adds new work — do NOT search the environment, the hub roster, or peer histories for another task, and do NOT invent follow-up work to keep going.
2. **Resume the work** — if the assignment is genuinely not finished and you are not recording an incremental section, call the next tool you would have called (edit, write, bash, search, etc.). NEVER treat this reminder as a forced stop.
3. **Yield an incremental section** — only when useful for the assignment: call `yield` with non-empty `type: string[]`; matching sections accumulate and the task continues.
...
Default to option 2 unless the work is actually done, actually blocked, or ready for an incremental section.
